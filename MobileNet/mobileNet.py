...
@Time    :   ${v1:2020-02-27} ${v2:2020-02-28}  ${v3:2020-03-01}
...
class SEBlock(nn.Module):
    def __init__(self,expand_size, divide = 4):
        ...
        self.dense = nn.Sequential(
            nn.Linear(expand_size, expand_size // divide),
            nn.ReLU(inplace=True),
            nn.Linear(expand_size  // divide, expand_size),
            HardSigmoid()
        )
...
class SEInvertResidual(nn.Module):
    def __init__(self, inchannel, outchannel, kernel_size, expand_size,SE,NL,stride):
        ...
        self.expansion = nn.Sequential(
            nn.Conv2d(in_channels=inchannel,out_channels=expand_size,kernel_size=1,stride=1),
            nn.BatchNorm2d(expand_size),
            activation(inplace=True)
        )

        self.depthwise = nn.Sequential(
            nn.Conv2d(in_channels=expand_size,out_channels=expand_size,kernel_size = kernel_size,
                      stride = stride,padding=padding,groups = expand_size),
            nn.BatchNorm2d(expand_size),
        )
        if SE:
        ...
        self.pointwise = nn.Sequential(
            nn.Conv2d(in_channels=expand_size,out_channels=outchannel,kernel_size = 1,stride = 1),
            nn.BatchNorm2d(outchannel),
            activation(inplace=True)
        )
    ...
    def __init__(self,net_mode ='large', multiplier = 1.0):
        ...
        if  net_mode == 'large':
            ...
            self.conv_nbn = nn.Sequential(
                ...
                HardSwish(inplace=True),
                nn.Conv2d(in_channels=1280, out_channels=5, kernel_size=1,stride=1)
            )

    def make_layer(self):
        bottlenecks = []
        for inchannel,outchannel,kernel_size,stride,activation,se,expand_size in self.cfgs:
            bottlenecks.append(SEInvertResidual(inchannel,outchannel,kernel_size,expand_size,se,activation,stride))
        return nn.Sequential(*bottlenecks)
